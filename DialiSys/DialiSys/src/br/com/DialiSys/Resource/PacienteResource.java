...
public class PacienteResource {
	...
	public Response EmitirRelatorioDoPaciente(@PathParam("id") long id, @QueryParam("DataInicial") String datainicial, 
		...
		List<BigDecimal> exames_ureiapre_list = new ArrayList<BigDecimal>();
		List<BigDecimal> exames_ureiapos_list = new ArrayList<BigDecimal>();
		...
		try{
			...
			if(interc_list.isEmpty()){
				...
				msgintercorrencias = "O Paciente Não teve nenhuma Intercorrência Registrada nas Sessões de Hemodiálise"
						+ "Buscadas";
		...
		try{
			...
			pesoE_list = query.getResultList();
				...
				pesoentrada.put("Peso_Entrada No Período", pesoE_list);
			
		}catch(Exception ex){
			
			ex.printStackTrace();
			System.out.println("Erro ao buscar os Pesos de Entrada das Sessões de Dialise");
		}
		...
		try{
			...
			pesoS_list = query.getResultList();
				...
				pesosaida.put("Peso_Saida No Período", pesoS_list);
			
		}catch(Exception ex){
			
			ex.printStackTrace();
			System.out.println("Erro ao buscar os pesos de saida das Sessões de Dialise");
		}
		...
		try{
			...
			ult_list = query.getResultList();
				...
				ultraf.put("Ultrafiltrações No Período", ult_list);
			
		}catch(Exception ex){
			
			ex.printStackTrace();
			System.out.println("Erro ao buscar as ultrafiltrações das Sessões de Dialise");
		}
		
		try{
			
			String consulta = "select e.ureiaPre from Sessao s join s.exameUreia e where s.paciente.pessoaf.id = :id_pessoa "
					+ "and s.dataSessao between :dataInicial and :dataFinal";
			TypedQuery<BigDecimal> query = em.createQuery(consulta, BigDecimal.class);
			query.setParameter("id_pessoa", id);
			query.setParameter("dataInicial", datainicio);
			query.setParameter("dataFinal", datafim);
			exames_ureiapre_list = query.getResultList();
			
			String consulta2 = "select e.ureiaPos from Sessao s join s.exameUreia e where s.paciente.pessoaf.id = :id_pessoa "
					+ "and s.dataSessao between :dataInicial and :dataFinal";
			TypedQuery<BigDecimal> query2 = em.createQuery(consulta2, BigDecimal.class);
			query2.setParameter("id_pessoa", id);
			query2.setParameter("dataInicial", datainicio);
			query2.setParameter("dataFinal", datafim);
			exames_ureiapos_list = query2.getResultList();
			
			if((exames_ureiapre_list.isEmpty()) && (exames_ureiapos_list.isEmpty())) {
				
				msgexameureia = "O Paciente Não teve nenhum Exame feito nas Sessões de Hemodiálise"
						+ "Buscadas";
				examesureia.putOpt("", msgexameureia);
				System.out.println(msgexameureia);
			}else{
				
				examesureia.put("Exames UreiaPre", exames_ureiapre_list);
				examesureia.put("Exames UreiaPos", exames_ureiapos_list);
			}
			
		}catch(Exception ex){
			
			ex.printStackTrace();
			System.out.println("Erro ao buscar os Exames das Sessões de Dialise");
		}
		...
		try{
			...
			pressoes_list = query.getResultList();
				...
				pressoes.put("Pressões Arteriais No Período", pressoes_list);
			
		}catch(Exception ex){
			
			ex.printStackTrace();
			System.out.println("Erro ao buscar as pressoes das Sessões de Dialise");
		}
		...
		try{
			...
			if(solucao_list.isEmpty()){
				...
				msgsolucao = "O Paciente Não precisou de nenhuma solução pois não teve Intercorrências nas Sessões de Hemodiálise"
						+ "Buscadas";
				solucao.putOpt("", msgsolucao);
				System.out.println(msgsolucao);
			}
				
				solucao.put("Soluções Para As Intercorrências No Período", solucao_list);
			
		}catch(Exception ex){
			
			ex.printStackTrace();
			System.out.println("Erro ao buscar as soluções de saida das Sessões de Dialise");
		}
		...
		try{
			...
			relatorio.putOpt("Ultrafiltrações", ultraf);
			relatorio.putOpt("Exames", examesureia);
			relatorio.putOpt("Pressões Arteriais", pressoes);
			...
			relatorio.putOpt("Soluções Para As Intercorrências Ocorridas Durante as Sessões", solucao);
